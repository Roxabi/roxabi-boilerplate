...
Vercel is the single deployment platform for all three applications. Production deploys happen automatically; previews are manual.

### Production Flow

Only merges to `main` trigger production deployments. `apps/web`, `apps/api`, and `apps/docs` are separate Vercel projects pointing to the same GitHub repository.
...
flowchart TD
    ...
    D3 --> D4["4. next build"]
    D4 --> D5["5. Deploy static/ISR site"]
```
