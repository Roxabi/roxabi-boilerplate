...
Where `FeatureFlagRow` is the inferred select type from the `featureFlags` schema table (`typeof featureFlags.$inferSelect`). This avoids introducing a separate domain type for a table that has no mapping layer today.
...
Where `SystemSettingRow` is the inferred select type from the `systemSettings` schema table.
...
- `FeatureFlagRow` and `SystemSettingRow` are schema-inferred types. If the project later introduces explicit domain types separate from the schema, the port signatures would need updating -- but that is a deliberate future decision, not an accidental coupling.
