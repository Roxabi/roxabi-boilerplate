...
- All methods accept `tx?: DrizzleTx` for optional transaction pass-through — this is a type-only import with no runtime cost; the service never passes a `tx`, but the parameter lets the adapter participate in caller-initiated transactions. This is a deliberate trade-off against the complexity of an ORM-agnostic `Transaction<T>` generic
