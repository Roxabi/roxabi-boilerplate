...
2. Add it as a GitHub Actions secret: `VERCEL_AUTOMATION_BYPASS_SECRET`
3. The Deploy Preview workflow injects the secret at web build time via `--build-env`
4. The Nitro proxy in `vite.config.ts` reads the secret and adds `x-vercel-protection-bypass` to every outgoing `/api/**` request (server-side, invisible to clients)
5. The health check step also uses the secret to verify API preview accessibility

The bypass secret is **only** active in preview environments (`VERCEL_ENV !== 'production'`), so production deployments are unaffected.
