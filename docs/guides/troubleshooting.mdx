...
**Symptom:** `connection refused` or `ECONNREFUSED` errors on port 5432 (or your `POSTGRES_PORT`) when starting the API or running migrations.

**Cause:** The PostgreSQL Docker container is not running, or is mapped to a different port than `DATABASE_URL` expects.
